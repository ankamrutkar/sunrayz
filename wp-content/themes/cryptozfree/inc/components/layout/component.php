<?php
/**
 * cryptozfree\Layout\Component class
 *
 * @package cryptozfree
 */

namespace cryptozfree\Layout;
 ...
 * * `cryptozfree()->get_layout()`
 * * `cryptozfree()->get_feature()`
 * * `cryptozfree()->show_feature()`
 * * `cryptozfree()->show_feature_above()`
 * * `cryptozfree()->show_feature_below()`
 * * `cryptozfree()->get_feature_position()`
 * * `cryptozfree()->show_comments()`
 * * `cryptozfree()->show_hero_title()`
 * * `cryptozfree()->show_in_content_title()`
 * * `cryptozfree()->get_boxed()`
 * * `cryptozfree()->has_sidebar()`
 * * `cryptozfree()->sidebar_id()`
 * * `cryptozfree()->sidebar_id_class()`
 * * `cryptozfree()->sidebar_side()`
 * * `cryptozfree()->display_sidebar()`
 * * `cryptozfree()->has_header()`
 * * `cryptozfree()->has_footer()`
 * * `cryptozfree()->has_content()`
 *
...
class Component implements Component_Interface, Templating_Component_Interface {
	...
	public function action_register_sidebars() {
		$widgets = array(
			'sidebar-primary'   => __( 'Sidebar 1', 'cryptozfree' ),
			'sidebar-secondary' => __( 'Sidebar 2', 'cryptozfree' ),
			'footer1'           => __( 'Footer 1', 'cryptozfree' ),
			'footer2'           => __( 'Footer 2', 'cryptozfree' ),
			'footer3'           => __( 'Footer 3', 'cryptozfree' ),
			'footer4'           => __( 'Footer 4', 'cryptozfree' ),
			'footer5'           => __( 'Footer 5', 'cryptozfree' ),
			'footer6'           => __( 'Footer 6', 'cryptozfree' ),
		);

		foreach ( $widgets as $id => $name ) {
			register_sidebar(
				apply_filters(
					'cryptozfree_widget_area_args',
					array(
						'name'          => $name,
						'id'            => $id,
	...
	public function display_sidebar() {
		...
		echo apply_filters( 'cryptozfree_dynamic_sidebar_content', ob_get_clean() ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
	/**
	...
	public static function show_hero_title() {
		if ( is_null( self::$layout ) ) {
			self::$layout = apply_filters( 'cryptozfree_post_layout', self::check_conditionals() );
		}
	...
	public static function desk_transparent_header() {
		...
		return ( 'enable' === self::$layout['transparent'] && cryptozfree()->sub_option( 'transparent_header_device', 'desktop' ) ? true : false );
	}
	/**
	 * Checks if page should have desktop transparent header.
	 *
	...
	public static function get_desk_transparent_class() {
		...
		return ( 'enable' === self::$layout['transparent'] && cryptozfree()->sub_option( 'transparent_header_device', 'desktop' ) ? 'transparent-header' : 'non-transparent-header' );
	}
	/**
	...
	public static function get_mobile_transparent_class() {
		...
		return ( 'enable' === self::$layout['transparent'] && cryptozfree()->sub_option( 'transparent_header_device', 'mobile' ) ? 'mobile-transparent-header' : 'mobile-non-transparent-header' );
	}
	/**
	 * Checks if page should have mobile transparent header.
	 *
	...
	public static function mobile_transparent_header() {
		...
		return ( 'enable' === self::$layout['transparent'] && cryptozfree()->sub_option( 'transparent_header_device', 'mobile' ) ? true : false );
	}
	/**
	...
	public static function check_conditionals() {
		...
		$footer      = 'enable';
		$content     = 'enable';
		$transparent = ( cryptozfree()->option( 'transparent_header_enable' ) ? 'enable' : 'disable' );
		if ( ( is_singular() || is_front_page() ) && ! is_home() ) {
			...
			if ( isset( $postsidebar ) && ! empty( $postsidebar ) && 'defualt' !== $postsidebar && 'default' !== $postsidebar ) {
				...
				$sidebar_id = cryptozfree()->option( $post_type . '_sidebar_id', $sidebar_id );
			}
			// Transparent.
			if ( isset( $posttrans ) && ( 'enable' === $posttrans || 'disable' === $posttrans ) ) {
				$transparent = $posttrans;
			} else {
				$option_trans = cryptozfree()->option( 'transparent_header_' . $trans_type, null );
			...
			if ( isset( $posttitle ) && ( 'above' === $posttitle || 'normal' === $posttitle || 'hide' === $posttitle ) ) {
				...
				$option_title_layout = cryptozfree()->option( $post_type . '_title_layout' );
				if ( 'above' === $option_title_layout || 'normal' === $option_title_layout ) {
					$title = $option_title_layout;
				}
			} else {
				$option_title = cryptozfree()->option( $post_type . '_title' );
			...
			if ( isset( $postvpadding ) && ( 'show' === $postvpadding || 'hide' === $postvpadding ) ) {
				...
				$option_vpadding = cryptozfree()->option( $post_type . '_vertical_padding' );
			...
			// Post Navigation.
			if ( 'post' === $post_type ) {
				$option_navigation = cryptozfree()->option( $post_type . '_navigation' );
				if ( $option_navigation ) {
					$navigation = 'show';
				}
			}
			// Post Comments.
			$option_comments = cryptozfree()->option( $post_type . '_comments' );
			if ( $option_comments ) {
				$comments = 'show';
			}
			...
			if ( isset( $postboxed ) && ( 'unboxed' === $postboxed || 'boxed' === $postboxed ) ) {
				...
				$option_boxed = cryptozfree()->option( $post_type . '_content_style' );
			...
			if ( isset( $postfeature ) && ( 'show' === $postfeature || 'hide' === $postfeature ) ) {
				...
				$option_feature = cryptozfree()->option( $post_type . '_feature' );
				if ( $option_feature ) {
					$feature = 'show';
				}
			}
			...
			if ( isset( $postf_position ) && ( 'above' === $postf_position || 'behind' === $postf_position || 'below' === $postf_position ) ) {
				...
				$option_f_position = cryptozfree()->option( $post_type . '_feature_position' );
			...
			if ( isset( $postlayout ) && ( 'narrow' === $postlayout || 'fullwidth' === $postlayout ) ) {
				...
				$option_layout = cryptozfree()->option( $post_type . '_layout' );
			...
			if ( isset( $archivesidebar ) && ! empty( $archivesidebar ) && 'default' !== $archivesidebar && 'defualt' !== $archivesidebar ) {
				...
				$sidebar_id = cryptozfree()->option( $archive_type . '_sidebar_id', $sidebar_id );
			}
			// Archive Transparent.
			if ( isset( $archivetrans ) && ( 'enable' === $archivetrans || 'disable' === $archivetrans ) ) {
				$transparent = $archivetrans;
			} else {
				$option_trans = cryptozfree()->option( 'transparent_header_' . $trans_type );
				if ( true === $option_trans ) {
					$transparent = 'disable';
				}
			}
			...
			if ( isset( $archivetitle ) && ( 'above' === $archivetitle || 'normal' === $archivetitle || 'hide' === $archivetitle ) ) {
				...
				$option_title = cryptozfree()->option( $archive_type . '_title' );
				if ( false === $option_title ) {
					$title = 'hide';
				} else {
					$option_title_layout = cryptozfree()->option( $archive_type . '_title_layout' );
					if ( empty( $option_title_layout ) ) {
						$option_title_layout = cryptozfree()->option( 'post_archive_title_layout' );
					}
					if ( 'above' === $option_title_layout || 'normal' === $option_title_layout ) {
						$title = $option_title_layout;
					}
			...
			if ( is_home() && is_front_page() ) {
				if ( ! cryptozfree()->option( 'post_archive_home_title' ) ) {
					$title = 'hide';
				}
			}
			if ( is_404() ) {
			...
			if ( isset( $archiveboxed ) && ( 'unboxed' === $archiveboxed || 'boxed' === $archiveboxed ) ) {
				...
				$option_boxed = cryptozfree()->option( $archive_type . '_content_style' );
				if ( empty( $option_boxed ) ) {
					$option_boxed = cryptozfree()->option( 'post_archive_content_style' );
				}
				if ( 'unboxed' === $option_boxed || 'boxed' === $option_boxed ) {
					$boxed = $option_boxed;
				}
			...
			if ( isset( $archivevpadding ) && ( 'show' === $archivevpadding || 'hide' === $archivevpadding ) ) {
				...
				$option_vpadding = cryptozfree()->option( $archive_type . '_vertical_padding' );
			...
			if ( isset( $archivelayout ) && ( 'narrow' === $archivelayout || 'fullwidth' === $archivelayout ) ) {
				...
				$option_layout = cryptozfree()->option( $archive_type . '_layout' );
				if ( empty( $option_layout ) ) {
					$option_layout = cryptozfree()->option( 'post_archive_layout' );
				}
				if ( 'narrow' === $option_layout || 'fullwidth' === $option_layout ) {
					$layout = $option_layout;
				}
	...
	public function filter_body_classes( array $classes ) : array {
		...
		if ( self::has_sidebar() ) {
			...
			if ( cryptozfree()->option( 'sidebar_sticky' ) ) {
				if ( cryptozfree()->option( 'sidebar_sticky_last_widget' ) ) {
