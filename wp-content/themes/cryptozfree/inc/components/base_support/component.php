<?php
/**
 * cryptozfree\Base_Support\Component class
 *
 * @package cryptozfree
 */

namespace cryptozfree\Base_Support;
 ...
 * * `cryptozfree()->get_version()`
 * * `cryptozfree()->get_post_types()`
 * * `cryptozfree()->get_asset_version( string $filepath )`
 */
class Component implements Component_Interface, Templating_Component_Interface {
	...
	public function initialize() {
		...
		//add_action( 'admin_notices', array( $this, 'cryptozfree_starter_templates_notice' ) );
		add_action( 'admin_notices', array( $this, 'cryptozfree_turn_off_gutenberg_plugin_notice' ) );
		add_action( 'wp_ajax_cryptozfree_dismiss_notice', array( $this, 'ajax_dismiss_starter_notice' ) );
		add_action( 'wp_ajax_cryptozfree_dismiss_gutenberg_notice', array( $this, 'ajax_dismiss_gutenberg_notice' ) );
		add_action( 'wp_ajax_cryptozfree_install_starter', array( $this, 'install_plugin_ajax_callback' ) );
	...
	/**
	 * Add Notice for cryptozfree Starter templates
	 */
	public function cryptozfree_starter_templates_notice() {
		if ( defined( 'cryptozfree_STARTER_TEMPLATES_VERSION' ) || get_option( 'cryptozfree_starter_plugin_notice' ) || ! current_user_can( 'install_plugins' ) ) {
			return;
		}
		$installed_plugins = get_plugins();
		if ( ! isset( $installed_plugins['cryptozfree-starter-templates/cryptozfree-starter-templates.php'] ) ) {
			$button_label = esc_html__( 'Install cryptozfree Starter Templates', 'cryptozfree' );
			$data_action  = 'install';
		} elseif ( ! self::active_plugin_check( 'cryptozfree-starter-templates/cryptozfree-starter-templates.php' ) ) {
			$button_label = esc_html__( 'Activate cryptozfree Starter Templates', 'cryptozfree' );
			$data_action  = 'activate';
		} else {
			return;
		}
		?>
		<div id="cryptozfree-notice-starter-templates" class="notice is-dismissible notice-info">
			<h2 class="notice-title"><?php echo esc_html__( 'Thanks for choosing the cryptozfree Theme!', 'cryptozfree' ); ?></h2>
			<p class="cryptozfree-notice-description"><?php /* translators: %s: <strong> <a> */ printf( esc_html__( 'Want to get started with a beautiful %1$sstarter template%2$s? Install the cryptozfree Starter Templates plugin to launch an optimized design in minutes.', 'cryptozfree' ), '<a href="https://wordpress.org/plugins/cryptozfree-starter-templates/" target="_blank">', '</a>', '<strong>', '</strong>' ); ?></p>
			<p class="install-submit">
				<button class="button button-primary cryptozfree-install-starter-btn" data-redirect-url="<?php echo esc_url( admin_url( 'themes.php?page=cryptozfree-starter-templates' ) ); ?>" data-activating-label="<?php echo esc_attr__( 'Activating...', 'cryptozfree' ); ?>" data-activated-label="<?php echo esc_attr__( 'Activated', 'cryptozfree' ); ?>" data-installing-label="<?php echo esc_attr__( 'Installing...', 'cryptozfree' ); ?>" data-installed-label="<?php echo esc_attr__( 'Installed', 'cryptozfree' ); ?>" data-action="<?php echo esc_attr( $data_action ); ?>"><?php echo esc_html( $button_label ); ?></button>
			</p>
		</div>
		<?php
		wp_enqueue_script( 'cryptozfree-starter-install' );
		wp_localize_script(
			'cryptozfree-starter-install',
			'cryptozfreeStarterInstall',
			array(
				'ajax_url'   => admin_url( 'admin-ajax.php' ),
				'ajax_nonce' => wp_create_nonce( 'cryptozfree-ajax-verification' ),
				'status'     => $data_action,
			)
		);
	}
	 ...
	 */
	public function cryptozfree_turn_off_gutenberg_plugin_notice() {
		if ( ! defined( 'GUTENBERG_VERSION' ) || get_option( 'cryptozfree_disable_gutenberg_notice' ) || ! current_user_can( 'install_plugins' ) ) {
			return;
		}
		?>
		<div id="cryptozfree-notice-gutenberg-plugin" class="notice is-dismissible notice-warning">
			<h2 class="notice-title"><?php echo esc_html__( 'Gutenberg Plugin Detected', 'cryptozfree' ); ?></h2>
			<p class="cryptozfree-notice-description"><?php /* translators: %s: <a> */ printf( esc_html__( 'The %1$sGutenberg plugin%2$s is not recommended for use in a production site. Many things may be broken by using this plugin. Please deactivate.', 'cryptozfree' ), '<a href="https://wordpress.org/plugins/gutenberg/" target="_blank">', '</a>' ); ?></p>
		</div>
		<?php
		wp_enqueue_script( 'cryptozfree-gutenberg-deactivate' );
		wp_localize_script(
			'cryptozfree-gutenberg-deactivate',
			'cryptozfreeGutenbergDeactivate',
			array(
				'ajax_url'   => admin_url( 'admin-ajax.php' ),
				'ajax_nonce' => wp_create_nonce( 'cryptozfree-ajax-verification' ),
			)
	...
	public function ajax_dismiss_gutenberg_notice() {
		...
		if ( ! isset( $_POST['action'] ) || 'cryptozfree_dismiss_gutenberg_notice' !== $_POST['action'] ) {
			return;
		}
		// Security check: Make sure nonce is OK.
		check_ajax_referer( 'cryptozfree-ajax-verification', 'security', true );

		// If we got this far, we need to dismiss the notice.
		update_option( 'cryptozfree_disable_gutenberg_notice', true, false );
	}
	/**
	 * Run check to see if we need to dismiss the notice.
	 * If all tests are successful then call the dismiss_notice() method.
	...
	public function ajax_dismiss_starter_notice() {
		...
		// Sanity check: Early exit if we're not on a wptrt_dismiss_notice action.
		if ( ! isset( $_POST['action'] ) || 'cryptozfree_dismiss_notice' !== $_POST['action'] ) {
			return;
		}
		// Security check: Make sure nonce is OK.
		check_ajax_referer( 'cryptozfree-ajax-verification', 'security', true );
		...
		// If we got this far, we need to dismiss the notice.
		update_option( 'cryptozfree_starter_plugin_notice', true, false );
	}
	...
	public function install_starter_script() {
		wp_register_script( 'cryptozfree-starter-install', get_template_directory_uri() . '/assets/js/admin-activate.min.js', array( 'jquery' ), VERSION, false );
		wp_register_script( 'cryptozfree-gutenberg-deactivate', get_template_directory_uri() . '/assets/js/gutenberg-notice.min.js', array( 'jquery' ), VERSION, false );
	}
	/**
	 * Active Plugin Check
	 *
	...
	public function install_plugin_ajax_callback() {
		if ( ! check_ajax_referer( 'cryptozfree-ajax-verification', 'security', false ) ) {
			wp_send_json_error( __( 'Security Error, Please reload the page.', 'cryptozfree' ) );
		}
		if ( ! current_user_can( 'install_plugins' ) ) {
			wp_send_json_error( __( 'Security Error, Need higher Permissions to install plugin.', 'cryptozfree' ) );
		}
		...
		if ( 'install' === $status ) {
			$api = plugins_api(
				'plugin_information',
				array(
					'slug' => 'cryptozfree-starter-templates',
					'fields' => array(
			...
			if ( ! is_wp_error( $api ) ) {
				...
				if ( $installed ) {
					$activate = activate_plugin( 'cryptozfree-starter-templates/cryptozfree-starter-templates.php', '', false, true );
		...
		if ( false === $install ) {
			wp_send_json_error( __( 'Error, plugin could not be installed, please install manually.', 'cryptozfree' ) );
		} else {
	...
	public function custom_excerpt_length( $length ) {
		...
		if ( is_search() ) {
			if ( cryptozfree()->sub_option( 'search_archive_element_excerpt', 'words' ) ) {
				$length = intval( cryptozfree()->sub_option( 'search_archive_element_excerpt', 'words' ) );
			}
		} else if ( 'post' === get_post_type() ) {
			if ( cryptozfree()->sub_option( 'post_archive_element_excerpt', 'words' ) ) {
				$length = intval( cryptozfree()->sub_option( 'post_archive_element_excerpt', 'words' ) );
			}
		} else {
			if ( cryptozfree()->sub_option( get_post_type() . '_archive_element_excerpt', 'words' ) ) {
				$length = intval( cryptozfree()->sub_option( get_post_type() . '_archive_element_excerpt', 'words' ) );
			}
		}
		return $length;
	}
	...
	public static function customizer_quick_link() {
		if ( is_customize_preview() ) {
			?>
			<div class="customize-partial-edit-shortcut cryptozfree-custom-partial-edit-shortcut">
				<button aria-label="<?php esc_attr_e( 'Click to edit this element.', 'cryptozfree' ); ?>" title="<?php esc_attr_e( 'Click to edit this element.', 'cryptozfree' ); ?>" class="customize-partial-edit-shortcut-button item-customizer-focus"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M13.89 3.39l2.71 2.72c.46.46.42 1.24.03 1.64l-8.01 8.02-5.56 1.16 1.16-5.58s7.6-7.63 7.99-8.03c.39-.39 1.22-.39 1.68.07zm-2.73 2.79l-5.59 5.61 1.11 1.11 5.54-5.65zm-2.97 8.23l5.58-5.6-1.07-1.08-5.59 5.6z"></path></svg></button>
			</div>
			<?php
		}
	}
	...
	public static function get_post_types_to_ignore() {
		if ( is_null( self::$ignore_post_types ) ) {
			...
			self::$ignore_post_types = apply_filters( 'cryptozfree_customizer_post_type_ignore_array', $ignore_post_types );
		}
	...
	public static function get_transparent_post_types_to_ignore() {
		if ( is_null( self::$transparent_ignore_post_types ) ) {
			...
			self::$transparent_ignore_post_types = apply_filters( 'cryptozfree_transparent_post_type_ignore_array', $transparent_ignore_post_types );
		}
	...
	public static function get_public_post_types_to_ignore() {
		if ( is_null( self::$public_ignore_post_types ) ) {
			...
			self::$public_ignore_post_types = apply_filters( 'cryptozfree_public_post_type_ignore_array', $public_ignore_post_types );
		}
	...
	public static function get_post_types() {
		if ( is_null( self::$post_types ) ) {
			...
			self::$post_types = apply_filters( 'cryptozfree_public_post_type_array', array_merge( $builtin, $post_types ) );
		}
	...
	public static function get_post_types_objects() {
		if ( is_null( self::$post_types_objects ) ) {
			...
			$operator         = 'and';
			$post_types       = get_post_types( $args, $output, $operator );
			self::$post_types_objects = apply_filters( 'cryptozfree_public_post_type_objects', $post_types );
		}
	...
	public function action_essential_theme_support() {
		if ( 'em' === cryptozfree()->sub_option( 'content_width', 'unit' ) || 'rem' === cryptozfree()->sub_option( 'content_width', 'unit' ) ) {
			$cryptozfree_content_width = intval( cryptozfree()->sub_option( 'content_width', 'size' ) * 17 );
		} else {
			$cryptozfree_content_width = cryptozfree()->sub_option( 'content_width', 'size' );
		}
		$GLOBALS['content_width'] = intval( $cryptozfree_content_width ); // phpcs:ignore WPThemeReview.CoreFunctionality.PrefixAllGlobals.NonPrefixedVariableFound
	...
	public function add_search_icon( $markup ) {
		$markup = str_replace( '</form>', '<div class="cryptozfree-search-icon-wrap">' . cryptozfree()->get_icon( 'search', '', false ) . '</div></form>', $markup );
		return $markup;
	}
	/**
	...
	public function filter_body_classes_add_link_style( array $classes ) : array {
		...
		$classes[] = 'link-style-' . cryptozfree()->sub_option( 'link_color', 'style' );
