<?php
/**
 * cryptozfree\Options\Component class
 *
 * @package cryptozfree
 */

namespace cryptozfree\Options;
 ...
 * * `cryptozfree()->option()`
 * * `cryptozfree()->default()`
 * * `cryptozfree()->get_option_type()`
 * * `cryptozfree()->get_option_name()`
 * * `cryptozfree()->sub_option()`
 * * `cryptozfree()->sidebar_options()`
 * * `cryptozfree()->palette_option()`
 * * `cryptozfree()->get_palette()`
 */
class Component implements Component_Interface, Templating_Component_Interface {
	...
	public function get_option_type() {
		...
		if ( is_null( self::$opt_type ) ) {
			self::$opt_type = apply_filters( 'cryptozfree_theme_option_type', 'theme_mod' );
		}
		// Return option_type.
	...
	public function get_option_name() {
		...
		if ( is_null( self::$opt_name ) ) {
			self::$opt_name = apply_filters( 'cryptozfree_theme_option_name', 'cryptozfree_settings' );
		}
		// Return option_name.
	...
	public static function defaults() {
		...
		if ( is_null( self::$default_options ) ) {
			self::$default_options = apply_filters(
				'cryptozfree_theme_options_defaults',
				array(
					'content_width'   => array(
						'size' => 1290,
						'unit' => 'px',
					...
					// Header HTML.
					'header_html_content'    => __( 'Insert HTML here', 'cryptozfree' ),
					...
					// Header Button.
					'header_button_label'      => __( 'Button', 'cryptozfree' ),
					'header_button_link'      => '',
					...
					'mobile_button_label'      => __( 'Button', 'cryptozfree' ),
					...
					// Mobile Header HTML.
					'mobile_html_content'    => __( 'Insert HTML here', 'cryptozfree' ),
					...
					'product_content_element_price' => array(
						'enabled' => true,
						'show_shipping' => false,
						'shipping_statement' => __( '& Free Shipping', 'cryptozfree' ),
					),
					...
					'product_content_element_extras' => array(
						'enabled'   => false,
						'title'     => __( 'Free shipping on orders over $50!', 'cryptozfree' ),
						'feature_1' => __( 'Satisfaction Guaranteed', 'cryptozfree' ),
						'feature_2' => __( 'No Hassle Refunds', 'cryptozfree' ),
						'feature_3' => __( 'Secure Payments', 'cryptozfree' ),
						'feature_4' => '',
						'feature_5' => '',
					...
					'product_content_element_payments' => array(
						'enabled' => false,
						'title'     => __( 'GUARANTEED SAFE CHECKOUT', 'cryptozfree' ),
						'visa' => true,
						'mastercard' => true,
						'amex' => true,
						'discover' => true,
	...
	public function get_palette_for_customizer() {
		$palette = get_option( 'cryptozfree_global_palette' );
	...
	public function palette_option( $subkey ) {
		...
		$active = apply_filters( 'cryptozfree_active_palette', ( self::$palette && is_array( self::$palette ) && isset( self::$palette['active'] ) && ! empty( self::$palette['active'] ) ? self::$palette['active'] : 'palette' ) );
		$value = '';
		...
		return apply_filters( 'cryptozfree_palette_option', $value, $subkey );
	}
