...
import cryptozfreeGradientPicker from './gradient-component.js';
import cryptozfreeColorPicker from '../common/color-picker';
...
class BackgroundComponent extends Component {
	constructor(props) {
		...
		let defaultParams = {
			...
			repeat: {
				'no-repeat': {
					name: __( 'No Repeat', 'cryptozfree' ),
				},
				'repeat': {
					name: __( 'Repeat', 'cryptozfree' ),
				},
				'repeat-x': {
					name: __( 'Repeat-X', 'cryptozfree' ),
				},
				'repeat-y': {
					name: __( 'Repeat-y', 'cryptozfree' ),
				},
			},
			size: {
				auto: {
					name: __( 'Auto', 'cryptozfree' ),
				},
				cover: {
					name: __( 'Cover', 'cryptozfree' ),
				},
				contain: {
					name: __( 'Contain', 'cryptozfree' ),
				},
			},
			attachment: {
				scroll: {
					name: __( 'Scroll', 'cryptozfree' ),
				},
				fixed: {
					name: __( 'Fixed', 'cryptozfree' ),
				},
			},
			attachments: {
				desktop: {},
		...
		const palette = JSON.parse( this.props.customizer.control( 'cryptozfree_color_palette' ).setting.get() );
		//console.log( palette );
		this.state = {
			value: value,
	...
	render() {
		...
		if( this.state.supportGradient ) {
			tab_options = [
				...
				},
				{
					...
					className: 'cryptozfree-image-background',
				},
			];
		} else {
			tab_options = [
		...
		return (
			<div className="cryptozfree-control-field cryptozfree-background-control">
				<ResponsiveControl
					onChange={ ( currentDevice) => this.setState( { currentDevice } ) }
					controlLabel={ controlLabel }
				>
					<div className="cryptozfree-background-picker-wrap">
						{ this.state.isVisible && (
							<Popover position="top left" className="cryptozfree-popover-color" onClose={ maybeToggleClose }>
												...
												if ( 'image' === tab.name ) {
													tabout = (
														<Fragment>
															...
															<span className="customize-control-title">{ __( 'Background Repeat', 'cryptozfree' ) }</span>
															...
															<span className="customize-control-title">{ __( 'Background Size', 'cryptozfree' ) }</span>
															<ButtonGroup className="cryptozfree-radio-container-control">
															...
															<span className="customize-control-title">{ __( 'Background Attachment', 'cryptozfree' ) }</span>
															<ButtonGroup className="cryptozfree-radio-container-control">
													...
													tabout = (
														<Fragment>
															{ this.state.isVisible && this.state.refresh && (
																<div className="cryptozfree-background-color-wrap">
						...
						<Tooltip text={ this.controlParams.tooltip ? this.controlParams.tooltip : __( 'Select Background', 'cryptozfree' ) }>
							<div className="background-button-wrap">
								<Button className={ 'cryptozfree-background-icon-indicate' } onClick={ () => { this.state.isVisible ? toggleClose() : toggleVisible() } }>
									{ ( undefined === this.state.value[ this.state.currentDevice ] || undefined === this.state.value[ this.state.currentDevice ].type || this.state.value[ this.state.currentDevice ].type === 'color' ) && (
										<Fragment>
											<ColorIndicator className="cryptozfree-advanced-color-indicate" colorValue={ getColorPreviewValue( this.state.currentDevice ) } />
											...
											{ showGradientPreview( this.state.currentDevice ) &&
												<ColorIndicator className="cryptozfree-advanced-color-indicate" colorValue={ getGradientPreviewValue( this.state.currentDevice ) } />
											}
										</Fragment>
									) }
	...
	preventClose() {
		let self = this;
		document.addEventListener( 'cryptozfreeOpenMediaModal', function(e) {
