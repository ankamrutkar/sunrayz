...
class BuilderComponent extends Component {
	...
	focusPanel( item ) {
		if ( undefined !== this.props.customizer.section( 'cryptozfree_customizer_' + item ) ) {
			this.props.customizer.section( 'cryptozfree_customizer_' + item ).focus();
		}
	}
	focusItem( item ) {
	...
	render() {
		return (
			<div className="cryptozfree-control-field cryptozfree-builder-items">
				...
				<div className="cryptozfree-builder-row-items">
	...
	// linkFocusButtons() {
	// 	this.props.control.container.on( 'click', '.cryptozfree-builder-areas .cryptozfree-builder-item', function( e ) {
	// 		e.preventDefault();
