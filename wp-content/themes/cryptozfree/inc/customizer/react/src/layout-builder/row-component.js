...
class RowComponent extends Component {
	render() {
		...
		let besideItems = [];
		return (
			<div className={ `cryptozfree-builder-areas ${ centerClass }` }>
				<Button
					className="cryptozfree-row-left-actions"
					aria-label={ __( 'Edit Settings for', 'cryptozfree' ) + ' ' + ( this.props.row === 'popup' ? __( 'Off Canvas', 'cryptozfree' ) : this.props.row + ' ' + __( 'Row', 'cryptozfree' ) ) }
					onClick={ () => this.props.focusPanel( mode + '_' + this.props.row ) }
					icon="admin-generic"
				>
				</Button>
				<Button
					className="cryptozfree-row-actions"
					aria-label={ __( 'Edit Settings for', 'cryptozfree' ) + ' ' + ( this.props.row === 'popup' ? __( 'Off Canvas', 'cryptozfree' ) : this.props.row + ' ' + __( 'Row', 'cryptozfree' ) ) }
					onClick={ () => this.props.focusPanel( mode + '_' + this.props.row ) }
				>
					{ ( this.props.row === 'popup' ? __( 'Off Canvas', 'cryptozfree' ) : this.props.row + ' ' + __( 'Row', 'cryptozfree' ) ) }
					<Dashicon icon="admin-generic" />
				</Button>
				<div className="cryptozfree-builder-group cryptozfree-builder-group-horizontal" data-setting={ this.props.row }>
