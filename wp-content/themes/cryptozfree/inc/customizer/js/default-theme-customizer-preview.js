( function( $ ) {
		...
		if ( hasSelectiveRefresh ) {
			wp.customize.selectiveRefresh.bind('partial-content-rendered', function (placement) {
				if ( placement.partial.id === 'header_desktop_items' ) {
					window.cryptozfree.initToggleDrawer();
				}
				window.cryptozfree.initTransHeaderPadding();
				if ( typeof window.cryptozfree.initStickyHeader !== "undefined" ) { 
					window.cryptozfree.initStickyHeader();
					window.cryptozfree.initScrollToTop();
				}
			} );
		}
	});
	var cryptozfreeCustomizer = {
		live_css_typography : function( key, rules, newValue ) {
			var styleID = 'cryptozfree-customize-preview-css-' + key,
			    $style = $( '#' + styleID ),
				css = '',
	...
	if ( cryptozfreeCustomizerPreviewData && cryptozfreeCustomizerPreviewData.liveControl ) {
		_.each( cryptozfreeCustomizerPreviewData.liveControl, function( liveControl, key ) {
			var eachLiveControl = {};
			...
			wp.customize( key, function( value ) {
				value.bind( function( newValue ) {
					_.each( eachLiveControl, function( rule, type ) {
						...
						cryptozfreeCustomizer[ functionName ]( key, rule, newValue );
					});
				});
			});
		});
	...
	$( document ).on( 'customize-preview-menu-refreshed', function( e, params ) {
        if ( params.wpNavMenuArgs.show_toggles ) {
			window.cryptozfree.initMobileToggleSub();
        }
	});
