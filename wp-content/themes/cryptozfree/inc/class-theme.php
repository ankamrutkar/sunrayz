...
class Theme {
	...
	/**
	 * Main cryptozfree\Theme Class Instance.
	 *
	 * Insures that only one instance of cryptozfree Class exists in memory at any one
	 * time. Also prevents needing to define globals all over the place.
	 *
	 * @static
	 * @static var array $instance
	 *
	 * @return cryptozfree\Theme Class
	 */
			...
			if ( ! $component instanceof Component_Interface ) {
				throw new InvalidArgumentException(
					sprintf(
						...
						__( 'The theme component %1$s does not implement the %2$s interface.', 'cryptozfree' ),
						gettype( $component ),
	...
	public function __clone() {
		...
		_doing_it_wrong( __FUNCTION__, esc_html__( 'Something went wrong.', 'cryptozfree' ), '1.0' );
	}
	 ...
	 *
	 * Calling `cryptozfree()` is a short-hand for calling this method on the main theme instance. The instance then allows
	...
	public function component( string $slug ) : Component_Interface {
		if ( ! isset( $this->components[ $slug ] ) ) {
			throw new InvalidArgumentException(
				sprintf(
					/* translators: %s: slug */
					__( 'No theme component with the slug %s exists.', 'cryptozfree' ),
					$slug
				)
			);
		}
